...
function updateBlogReferences() {
  ...
  blogPosts.forEach(blogPost => {
    try {
      ...
      updatedContent = updatedContent.replace(singleImageRegex, (match, blogSlug, imagePath) => {
      ...
      const gridImageRegex = /src:\s*["']\/blog\/([^\/]+)\/images\/([^"']+)["']/g;
      updatedContent = updatedContent.replace(gridImageRegex, (match, blogSlug, imagePath) => {
        postReferencesUpdated++;
        return `src: "/images/blog/${blogSlug}/${imagePath}"`;
      });
      ...
      if (content !== updatedContent) {
        ...
        console.log(`✓ Updated ${postReferencesUpdated} references in ${blogName}`);
...
function addBlogSlugParameter() {
  ...
  blogPosts.forEach(blogPost => {
    try {
      ...
      updatedContent = updatedContent.replace(twoColGridRegex, (match, attributes) => {
      ...
      updatedContent = updatedContent.replace(threeColGridRegex, (match, attributes) => {
        if (!attributes.includes('blogSlug=')) {
          postComponentsUpdated++;
      ...
      updatedContent = updatedContent.replace(singleImageRegex, (match, attributes) => {
        if (!attributes.includes('blogSlug=')) {
          postComponentsUpdated++;
      ...
      if (content !== updatedContent) {
        // Write the updated content back to the file
        fs.writeFileSync(blogPost, updatedContent, 'utf8');
        console.log(`✓ Added blogSlug to ${postComponentsUpdated} components in ${blogName}`);
      ...
      console.error(`✗ [Error] Failed to update components in ${blogPost}: ${error.message}`);
      stats.errors++;
    }
  });
...
console.log('\nNext steps:');
console.log('1. Check your blog posts to ensure images are displaying correctly');

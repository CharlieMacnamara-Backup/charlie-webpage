...
function checkMdxSyntax() {
  ...
  const syntaxPatterns = [
    ...
    {
      pattern:
        /<(SingleImage|TwoColGrid|ThreeColGrid)[^>]*>(?![\s\S]*?<\/\1>)(?![\s\S]*?\/\s*>)/g,
    ...
    try {
      ...
      if (fileIssues > 0) {
        ...
        if (updatedContent !== content) {
          ...
          console.log(
            `   Fixed issues automatically and saved backup to ${backupPath}`,
          )
        } else {
...
if (stats.warnings.length > 0) {
  ...
  console.log('\nNext steps:')
  console.log(
    '1. Review each file with issues and fix them manually if they were not automatically fixed',
  )
  console.log(
    '2. Use proper JSX syntax: self-closing tags should be <Component prop="value" />',
  )

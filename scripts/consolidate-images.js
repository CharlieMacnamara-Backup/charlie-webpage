...
function consolidateImageSystem() {
  ...
  const srcImages = glob.sync(
    'src/app/blog/*/images/**/*.{jpg,jpeg,png,gif,webp,svg,heic,mp4,mov}',
    { nocase: true },
  )
  ...
  const publicImages = glob.sync(
    'public/images/blog/**/*.{jpg,jpeg,png,gif,webp,svg,heic,mp4,mov}',
    { nocase: true },
  )
    ...
    try {
      ...
      if (fs.existsSync(targetPath)) {
        ...
        if (areFilesIdentical(srcImage, targetPath)) {
          ...
          console.log(
            `! [Conflict] Different versions exist: ${srcImage} and ${targetPath}`,
          )
...
console.log(
  '2. Update any remaining image references in your blog files to use the standard format:',
)

...
export default function TableOfContents({ className = '' }) {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const elements = Array.from(articleElement.querySelectorAll('h2, h3'))
        .filter(
          (element) =>
            element.textContent.trim().toLowerCase() !== 'table of contents',
        )
        .map((element) => {
          // Add ID if missing
          if (!element.id) {
  ...
  return (
    <div
      className={`my-8 rounded-xl border border-zinc-200 bg-white dark:border-zinc-700/40 dark:bg-zinc-900/70 ${className}`}
    >
